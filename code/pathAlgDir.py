...
rho = 4./k
print np.mean([np.sum(w) for w in W])
rho = edge_diff(x_star,W) / np.sum(x_star)
...
W = W.tolist()
C = make_C(y,W,rho,4.3)
val, F, path = edmonds_karp(C,0,1)
C_star = EK_bfs(C,F,0,1)[1]

C_star = [(i - 2)*n1 + i for i in range(n1) if i > 2] + [(i - 2)*n1 + i+1 for i in range(n1) if n1 -1 > i > 2] + [(i - 2)*n1 + i+2 for i in range(n1) if n1 -2 > i > 2]
C_star = []
for i in range(6):
    C_star += [(i+1)*n1 + n1 - j - 1 for j in range(6 - i)]
x_star = np.zeros(n)
x_star[list(C_star)] = 1.
...
mu = 8. / (np.sum(x_star))**0.5
y = mu*x_star + np.random.normal(0,sigma,n)
...
def dir_image(x,filename):
    ...
    for u1 in range(n1-1):
        for u2 in range(n1-1):
            pyplot.arrow(u1+.25,u2+.25,.5,.5,width=.05,fc='w')
    pyplot.savefig(filename,dpi=100)

    
dir_image(y, 'dir_n=15_noisy.png')
dir_image(x_star, 'dir_n=15_true.png')
dir_image(x_best, 'dir_n=15_hat.png')
